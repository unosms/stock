                ...
                <table class="min-w-full table-fixed divide-y divide-slate-200" style="width:100%; table-layout:fixed; border-collapse:collapse;">
                    ...
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-xs font-semibold uppercase text-slate-500" style="text-align:left;">Source</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($purchases as $purchase)
                            <tr>
                                <td class="px-4 py-3 align-middle text-sm text-slate-700 whitespace-nowrap" style="text-align:left;">{{ $purchase->purchased_at?->format('Y-m-d H:i') }}</td>
                                <td class="px-4 py-3 align-middle text-sm text-slate-900" style="text-align:left;">{{ $purchase->product?->name }}</td>
                                <td class="px-4 py-3 align-middle text-sm text-slate-700" style="text-align:left;">{{ $purchase->source?->name ?? 'N/A' }}</td>
                                <td class="px-4 py-3 align-middle text-sm text-slate-700 whitespace-nowrap" style="text-align:right;">{{ number_format($purchase->unit_price, 2) }}</td>
                                <td class="px-4 py-3 align-middle text-sm text-slate-900 whitespace-nowrap" style="text-align:right;">{{ number_format($purchase->quantity) }}</td>
                                <td class="px-4 py-3 align-middle text-sm font-semibold text-slate-900 whitespace-nowrap" style="text-align:right;">{{ number_format($purchase->total_price, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
